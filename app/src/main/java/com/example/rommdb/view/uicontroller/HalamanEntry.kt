...
fun FormInputSiswa(
    ...
    Column(
        modifier = modifier,
        verticalArrangement = Arrangement.spacedBy(space = dimensionResource(id = 16dp))
    ) {
        OutlinedTextField(
        ...
        if (enabled) {
            Text(
                ...
                modifier = Modifier.padding(start = dimensionResource(id = 16dp))
            )
        }
        HorizontalDivider(
            modifier = Modifier.padding(bottom = dimensionResource(16dp)),
            thickness = dimensionResource(8dp),
            color = Color.Blue
        )
    }
}
